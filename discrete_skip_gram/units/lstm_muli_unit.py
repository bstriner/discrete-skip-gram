...
from ..layers.utils import build_pair, build_bias, build_kernel


class LSTMMultiUnit(object):
    def __init__(self, model, input_units, units, name):
        self.inputs = len(input_units)
        bias = build_bias(model, (units,), "{}_b".format(name))
        Wh = build_kernel(model, (units, units), "{}_Wh".format(name))
        Ws = [build_kernel(model, (iu, units), "{}_W_{}".format(name, i)) for i, iu in enumerate(input_units)]
