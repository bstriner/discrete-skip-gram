...
class OneBitBayesModel(SkipgramModel):
    def __init__(self,
        ...
        prior_nll = Lambda(lambda (_p, _y): -T.log(_p[T.arange(_p.shape[0]).reshape((-1,1)), _y]),
        ...
        rs = Reshape((z_k,))
        sm = softmax_nd_layer()
        zx_p = UniformSmoothing()(sm(rs(x_embedding(input_x))))  # (n, z_k)

        # p(z)
        z_p = T.mean(zx_p, axis=0) # (z_k,)
